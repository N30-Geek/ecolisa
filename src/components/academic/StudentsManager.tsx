...
// ── PAGE DÉDIÉE PROFIL ÉLÈVE COMPLET ──
const StudentDetailPage: React.FC<{ student: Eleve; onBack: () => void }> = ({ student, onBack }) => {
  const [tab, setTab] = useState<'identity' | 'medical' | 'parents'>('identity');
  ...
  return (
    <div className="space-y-6 animate-fade-in select-none">
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-[0.97] text-white font-bold text-xs shadow-md shadow-indigo-500/25 flex items-center gap-2 transition-all duration-200 cursor-pointer"
          >
            <FileText className="w-4 h-4 text-white" /> Exporter Dossier PDF
          </button>
          <button
          ...
          >
            <Eye className="w-4 h-4 text-white" /> Aperçu Carte QR
          </button>
        </div>
      </div>
      ...
      <div
        className="p-6 rounded-2xl border-0 shadow-lg shadow-indigo-500/5 space-y-4"
        style={{ background: 'var(--bg-surface)' }}
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            {student.photoUrl ? (
              <img src={student.photoUrl} alt={student.prenom} className="w-16 h-16 rounded-2xl object-cover shadow-sm shrink-0" />
            ) : (
              <div
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                Classe: <span className="text-indigo-600 dark:text-indigo-400 font-bold">{student.nomClasse}</span> · Sexe: <span className="font-bold text-slate-700 dark:text-slate-200">{student.sexe === 'M' ? 'Masculin' : 'Féminin'}</span>
              </p>
              <div className="flex items-center gap-2 pt-0.5 flex-wrap">
                ...
                <span className="text-[11px] font-medium px-2.5 py-0.5 rounded-md" style={{ background: 'var(--bg-sunken)', color: 'var(--text-secondary)' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="p-6 rounded-2xl border-0 shadow-md space-y-4" style={{ background: 'var(--bg-surface)' }}>
          <h3 className="text-sm font-black uppercase tracking-wider text-indigo-500 flex items-center gap-2">
            <User className="w-4.5 h-4.5" /> Fiche d'État Civil & Naissance
          </h3>
          <div className="space-y-3 text-xs">
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
              <span className="font-bold text-slate-400">Nom :</span>
              <span className="font-black" style={{ color: 'var(--text-primary)' }}>{student.nom}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
              <span className="font-bold text-slate-400">Postnom :</span>
              <span className="font-black" style={{ color: 'var(--text-primary)' }}>{student.postnom || '—'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
              <span className="font-bold text-slate-400">Prénom :</span>
              <span className="font-black text-indigo-500">{student.prenom}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
              <span className="font-bold text-slate-400">Nationalité :</span>
              <span className="font-black text-emerald-600 dark:text-emerald-400">{student.nationalite || 'Congolaise (RDC)'}</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
            ...
            <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
              <span className="font-bold text-slate-400">Adresse Physique :</span>
              <span className="font-bold text-slate-700 dark:text-slate-300">{student.adressePhysique || 'Kinshasa'}</span>
            </div>
          </div>
        </div>

        {/* Fiche Tuteurs & Contacts */}
        <div className="p-6 rounded-2xl border-0 shadow-md space-y-4" style={{ background: 'var(--bg-surface)' }}>
          <h3 className="text-sm font-black uppercase tracking-wider text-indigo-500 flex items-center gap-2">
            <Users className="w-4.5 h-4.5" /> Contacts Parents & Tuteurs
          </h3>
          <div className="space-y-3 text-xs">
            <div className="p-3 rounded-xl space-y-1" style={{ background: 'var(--bg-sunken)' }}>
              <p className="font-black text-sm" style={{ color: 'var(--text-primary)' }}>{student.nomParent}</p>
              <p className="text-xs font-bold text-indigo-500">Tuteur Légal Principal</p>
              <div className="flex items-center gap-2 pt-1 font-mono font-bold text-indigo-600 dark:text-indigo-400">
                <Phone className="w-3.5 h-3.5" />
                <a href={`tel:${student.telephoneParent}`}>{student.telephoneParent}</a>
              </div>
            </div>
            {student.emailParent && (
              <div className="flex justify-between py-1.5 border-b border-slate-100 dark:border-slate-800/40">
                <span className="font-bold text-slate-400">Email Parent :</span>
                <span className="font-mono font-bold text-indigo-500">{student.emailParent}</span>
              </div>
            )}
          </div>
        </div>
